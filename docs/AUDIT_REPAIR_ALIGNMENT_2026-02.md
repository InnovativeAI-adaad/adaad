...
13. **README + docs integrity guards**
   ...
   - Added optional human-facing image-weight marker enforcement in `scripts/validate_docs_integrity.py` (`--enforce-image-weight-markers`) to require approved low/critical markers on HTML `<img>` embeds in `README.md` and `docs/` (configurable via `--image-weight-roots`).
