# Phase 96 Plan — INNOV-12: Mutation Genealogy Visualization

## Objective
Deliver **INNOV-12 (Mutation Genealogy Visualization)** as a governed phase increment with deterministic implementation sequencing, constitutional gate coverage, and release evidence alignment.

## Dependency Chain
- Immediate predecessor: **Phase 95** (must be merged before any Phase 96 source writes).
- Innovation lineage: `INNOV-11` → `INNOV-12` within `ADAAD_30_INNOVATIONS.md` phase roadmap (INNOV-10..30 mapped to Phases 94..114).
- Release dependency: target version **v9.29.0** follows predecessor semantic progression.
- Deterministic next-PR linkage: `state_alignment.expected_next_pr` resolves to `Phase 96` only when Phase 95 status is `shipped`.
...
- **PHASE-96-SEQ-0**: predecessor linkage is explicit and linear (`Phase 95` → `Phase 96`).
- **EVIDENCE-96-0**: no closure without complete claims-evidence row and resolvable artifacts.
...
- `docs/comms/claims_evidence_matrix.md` row updated to `Complete` for Phase 96 / INNOV-12.
...
3. **Release checkpoint:** governor records signed ledger attestation for `v9.29.0`.
